...
/**
 * Exchanger - это синхронизатор который позволяет запускать действия в нескольких потоках одновременно,
 * иногда используется для синхронизации потоков (что бы запустить потоки в одно время)
 * <p>
 * Допустим у нас есть два грузовика Грузовик1 едет по дороге А->D, Грузовик2 едет по дороге B->C,
 * грузовики должны встретится на перекрестке E и обменятся товарами которые должны ехать по другой дороге,
 * для Грузовика1 это посылка A->C для Грузовика2 это посылка B->D
 */
...
public class ExchangerEx {
    ...
    public static class Truck implements Runnable {
        private int number; //Номер грузовика
        private String dep; //Отправитель
        private String dest; //Пункт назначения
        private String[] parcels; //Посылки
